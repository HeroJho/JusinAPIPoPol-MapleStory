...
void CNightE::Initialize(void)
{
	CBmpMgr::Get_Instance()->Insert_Bmp(L"../Image/Monster/NightE/NightEL.bmp", L"NightEL");
	CBmpMgr::Get_Instance()->Insert_Bmp(L"../Image/Monster/NightE/NightER.bmp", L"NightER");
	Set_FrameKey(L"NightEL");
...
void CNightE::Release(void)
{

}

void CNightE::SetCurState(STATE _eState, DIRECTION _eDir)
{
	if (_eDir == DIR_LEFT)
		m_pFrameKey = L"NightEL";
	else
		m_pFrameKey = L"NightER";
...
void CNightE::Update_Idle()
{
}

void CNightE::Update_Walk()
{
	if (m_eDir == DIR_LEFT)
		m_tInfo.fX -= m_fSpeed;
...
void CNightE::Update_Chase()
{
	...
	if (m_fAttackRange > dis)
	{
		...
		((CSkill*)pSkill)->Set_ColSet(120.f, 80.f, 100.f, -35.f, 1100.f, 600.f);

		CObjMgr::Get_Instance()->Add_Object(OBJ_MONSKILL, pSkill);
	}
}

void CNightE::Update_Hit()
{
...
void CNightE::OnePlayEnd(void)
{

}

void CNightE::Motion_Change(void)
{
