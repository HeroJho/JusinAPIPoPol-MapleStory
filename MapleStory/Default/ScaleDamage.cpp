...
void CScaleDamage::Init(int _iStart, int _iEnd, int _iDamageSkin)
{
	m_iStart = _iStart;
	m_iEnd = _iEnd;
...
void CScaleDamage::Initialize(void)
{
	...
	m_fOldScaleTime = GetTickCount64();
	m_fScaleTime = 50.f;
	m_iScale = -40;
}

int CScaleDamage::Update(void)
{
	if (m_bDead)
		return OBJ_DEAD;
	...
	if (m_fOldScaleTime + m_fScaleTime < GetTickCount64())
	{
		m_iScale += 5;
		m_fOldScaleTime = GetTickCount64();
	}
...
void CScaleDamage::Late_Update(void)
{
...
void CScaleDamage::Render(HDC hDC)
{
	int iShfX = 0;
	...
	for (int i = 0; i < m_sDamage.size(); ++i)
	{
		...
		GdiTransparentBlt(hDC,
			...
			int(m_tInfo.fTCX) + m_iScale,
			int(m_tInfo.fTCY) + m_iScale,
			hMemDC,
...
void CScaleDamage::Release(void)
{
}
