...
void CDeletePig::Initialize(void)
{
	...
	m_tFrame.dwTime = (LONG)GetTickCount64();
...
void CDeletePig::DropItem()
{
	CObjMgr::Get_Instance()->Add_Object(OBJ_ITEM, CAbstractFactory<CMoneyBig>::Create(m_tInfo.fX, m_tInfo.fY - 10.f, "Item"));
	CObjMgr::Get_Instance()->Add_Object(OBJ_ITEM, CAbstractFactory<CRedPosion>::Create(m_tInfo.fX, m_tInfo.fY - 10.f, "Item"));
}
