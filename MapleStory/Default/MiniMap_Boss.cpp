...
void CMiniMap_Boss::Initialize(void)
{
	m_sTag = "MiniMap_2";
...
void CMiniMap_Boss::Late_Update(void)
{

}

void CMiniMap_Boss::Render(HDC hDC)
{

	HDC		hMemDCFrame = CBmpMgr::Get_Instance()->Find_Image(L"MiniMap_Boss");
	...
	float fDisX = fMapX / 158;
	float fDisY = fMapY / 90;
	INFO pPlayerPos = CObjMgr::Get_Instance()->Get_Player()->Get_Info();


	GdiTransparentBlt(hDC,
		int(m_tTRect.left + pPlayerPos.fX / fDisX) + 12,
...
void CMiniMap_Boss::ColRender(HDC hDC)
{
	Rectangle(hDC,
		int(m_tRect.left),
		int(m_tRect.top),
...
void CMiniMap_Boss::Release(void)
{
}