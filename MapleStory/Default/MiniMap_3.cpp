...
void CMiniMap_3::Initialize(void)
{
	m_sTag = "MiniMap_2";
...
void CMiniMap_3::Late_Update(void)
{

}

void CMiniMap_3::Render(HDC hDC)
{

	HDC		hMemDCFrame = CBmpMgr::Get_Instance()->Find_Image(L"MiniMap_3");
	...
	float fDisX = fMapX / 333;
	float fDisY = fMapY / 96;
	INFO pPlayerPos = CObjMgr::Get_Instance()->Get_Player()->Get_Info();


	GdiTransparentBlt(hDC,
		int(m_tTRect.left + pPlayerPos.fX / fDisX) + 22,
		int(m_tTRect.top + pPlayerPos.fY / fDisY) + 79,
		10,
		10,
		hMemDCFrame,
		0,
...
void CMiniMap_3::ColRender(HDC hDC)
{
	Rectangle(hDC,
		int(m_tRect.left),
		int(m_tRect.top),
